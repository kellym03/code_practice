...
#LRU stands for "least recently used." It's a type of cache that stores items up to a certain size limit. 
# When it gets full, it makes space for new items by discarding the least recently used items first. 
# The cache can be effective because items that are used a lot – like frequently repeated calls to the same function – are less likely to be discarded. They stay in-cache.

#The lru_cache decorator memoizes the inputs and outputs of the decorated function. 
# It speeds up repeated calls to a slow function with the same inputs.
# A function that reads from disk, makes network requests, or requires a lot of computation could be a good candidate for LRU caching if it also sees many identical calls.
...
#Because the factorial function is recursive and the inputs are sequential numbers, it does get called repeatedly with the same inputs. 
# Without caching, the function would be called 30 times in the code above. 
